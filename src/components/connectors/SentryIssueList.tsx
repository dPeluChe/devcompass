...
function IssueRow({ iss }: { iss: SentryIssue }) {
  return (
    <li className="connector-issue">
 ...
 * org-wide connector test where issues span projects); the per-repo Sentry tab
 * leaves it flat since it's already scoped to one project.
 */
export function SentryIssueList({ issues, groupByProject = false }: { issues: SentryIssue[]; groupByProject?: boolean }) {
  if (!groupByProject) {
    return <ul className="connector-issue-list">{issues.map((iss) => <IssueRow key={iss.id} iss={iss} />)}</ul>
  }
    ...
    <div className="sentry-issue-groups">
      {[...groups.entries()].map(([slug, list]) => (
        <div key={slug} className="sentry-issue-group">
          ...
          <ul className="connector-issue-list">{list.map((iss) => <IssueRow key={iss.id} iss={iss} />)}</ul>
        </div>
      ))}
    </div>
  )
